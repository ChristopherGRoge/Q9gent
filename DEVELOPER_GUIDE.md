...
```
Client                    Q9gent                    Claude CLI         SessionStore
  │                         │                           │                    │
  │  POST /spawn           │                           │                    │
  │  {create_session:true} │                           │                    │
  ├────────────────────────>│                           │                    │
  ...
  │                         │  (parse session_id)      │                    │
  │                         │                           │                    │
  │                         │  save_session(claude_session_id)              │
  │                         ├───────────────────────────────────────────────>│
  │                         │                           │                    │
  │  SSE: session_created  │                           │                    │
  │<────────────────────────┤                           │                    │
  │  {session_id: "..."}   │  (Claude's session_id)   │                    │
  │                         │                           │                    │
  │  [Continue streaming Claude output...]             │                    │
  │                         │                           │                    │
  │  POST /message/        │                           │                    │
  │       {session_id}     │                           │                    │
  ├────────────────────────>│                           │                    │
  ...
  │                         ├───────────────────────────────────────────────>│
  │                         │                           │                    │
  │                         │  spawn with --resume <session_id>             │
  │                         ├──────────────────────────>│                    │
  │                         │  JSONL stream (context)  │                    │
  │                         │<──────────────────────────┤                    │
```
...
pub struct SessionMetadata {
    pub session_id: String,      // Claude's session_id (UUID v4 from Claude CLI)
...
**Important:** The `session_id` is **extracted from Claude's JSONL output**, not generated by Q9gent. When `create_session=true`, Q9gent:
1. Spawns Claude without `--resume` flag
2. Parses the first JSONL line (system init event)
3. Extracts Claude's `session_id` field
4. Creates session metadata using Claude's session_id
...
1. **Create (from Claude's session_id):**
   ```rust
   // Parse JSONL from Claude to extract session_id
   let parsed = serde_json::from_str::<serde_json::Value>(&first_line)?;
   let claude_session_id = parsed.get("session_id").unwrap().as_str().unwrap();

   // Create metadata with Claude's session_id
   ...
   SessionStore::save_session(&metadata).await?;
   ```
...
**Note:** Q9gent no longer calls `create_session()` before spawning Claude. Session creation happens AFTER receiving Claude's first JSONL line.
...
## Architecture Changes (v0.1.3)

### Critical Session Handling Fix

**Issue in v0.1.2 and earlier:**
Q9gent was incorrectly using the `--resume` flag on initial session creation, causing all stateful operations to fail.

**Root Cause:**
```rust
// src/api.rs:120 (v0.1.2 - BROKEN)
resume_id: payload.resume_id.or_else(|| session_id.clone())
```

This code would:
1. Generate a Q9gent UUID when `create_session=true`
2. Immediately pass it to Claude with `--resume <new-uuid>`
3. Claude would fail: "No conversation found with session ID"

**Solution in v0.1.3:**
Complete architectural refactor to extract and use Claude's actual session_id:

1. **Removed premature UUID generation** - Don't create session_id before spawning Claude
2. **Parse Claude's JSONL output** - Extract session_id from first line (system init event)
3. **Use Claude's session_id** - Store and track Claude's session_id instead of Q9gent's
4. **Proper --resume usage** - Only use `--resume` when actually resuming (via `/message/:id`)

**Code Flow (v0.1.3):**
```rust
// src/api.rs - NEW FLOW
// 1. Spawn Claude WITHOUT --resume
let (child, rx) = agent_runner.spawn(request).await?;

// 2. Parse first JSONL line
if let Ok(parsed) = serde_json::from_str::<Value>(&first_line) {
    if let Some(sid) = parsed.get("session_id").and_then(|v| v.as_str()) {
        // 3. Create metadata with Claude's session_id
        let metadata = SessionMetadata {
        ...
        session_store.save_session(&metadata).await?;

        // 4. Send session_created event with Claude's session_id
        yield session_created_event(sid);
    }
}
```
...
- ✅ Session resumption with `--resume` now functional
...
**Version History:**
- **0.1.3** (2025-11-03) - Critical session handling fix: Extract and use Claude's session_id instead of Q9gent UUIDs
